...
const RELEASE_ASSET_NAME: &str = "claude-codex-usage-monitor.exe";
...
const WINGET_PACKAGE_ID: &str = "Chinzoe.ClaudeCodexUsageMonitor";
...
fn updates_dir() -> Result<PathBuf, String> {
    dirs::data_local_dir()
        .map(|dir| dir.join("ClaudeCodexUsageMonitor").join("updates"))
        .or_else(|| {
            Some(
                std::env::temp_dir()
